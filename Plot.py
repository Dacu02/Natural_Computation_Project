...
def summary_plots(problem_folder: str, algorithms: list[str], results_per_algorithm: dict[str, np.ndarray|None], problem: int, seeds: list[int]) -> None:
...
def main():
    ...
    for folder in sub_folders:
        if 'config.txt' not in os.listdir(folder):
            raise ValueError(f"Il percorso {data_path} non contiene cartelle di algoritmi")
    ...
    for algorithm in chosen_algorithms:
        ...
        final_results_file = os.path.join(alg_folder, 'final_results.csv')
        if not os.path.exists(final_results_file):
            print(f"Attenzione: il file {final_results_file} non esiste. L'algoritmo {algorithm} verrà saltato.")
            results_per_algorithm[algorithm] = None
            continue
        df = read_csv(final_results_file)
        final_errors = df['FinalError'].values
        results_per_algorithm[algorithm] = final_errors
